<template>
  <div class="login-wrap">
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
      <h3 class="title">Tallybook Login</h3>
...
export default {
  ...
  created() {
    ...
    //获取vuex中的token 如果不为空则证明此次路由是用户信息失效而发起的
    if(this.$route.params.expire == 1){
      this.$message({
          type: "error",
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          //   }
          // })
          tranReq("post", this.ruleForm, "/jwt/login/" + this.ruleForm.username + "/" + this.ruleForm.password).then((res) => {
            if(res.code == 200){
              window.vuethis = this;
              setTimeout(() => {
                this.$store.commit('setToken', res.data);
                window.sessionStorage.setItem("token", res.data);
                this.$router.push({ path: '/moneytable' })
              }, 1000)
            } else {
              this.$message({
                message: res.message,
